...
export const Route = createFileRoute('/(web)/cabins/$cabinSlug')({
    ...
    loader: async ({ context, params }) => {
        const data = await context.queryClient.ensureQueryData(cabinQueries.getBySlug(params.cabinSlug))
...
function RouteComponent() {
    const { cabin, amenities, recommended } = Route.useLoaderData()
    ...
    const rating = cabin.reviews.reduce((acc, review) => acc + review.rating, 0) / cabin.reviews.length
        ...
        <>
            <section className="mx-auto max-w-7xl px-8 pt-12 pb-16">
                ...
                <div className="relative h-[60vh] overflow-hidden rounded-xl shadow">
                    <img src="/placeholder.jpg" alt={cabin.name} className="h-full w-full object-cover" />
                    <div className="absolute inset-0 bg-gradient-to-t from-black/90 to-transparent"></div>
                    <div className="absolute bottom-6 left-6">
                        <Typography variant="h1" size="xl" className="text-4xl font-bold">
                            {cabin.name}
                        </Typography>
                        <Typography className="mt-2 text-base">{cabin.summary}</Typography>
                    </div>
                </div>
                ...
                <div className="mt-12 grid grid-cols-1 gap-12 md:grid-cols-3">
                    ...
                    <div className="space-y-10 leading-relaxed md:col-span-2">
                        <div className="space-y-4">
                            ...
                            <Typography className="text-base">{cabin.description}</Typography>
                        </div>
                    ...
                    <div className="sticky top-6 space-y-6 self-start rounded-xl border p-6 shadow md:col-span-1">
                        ...
                        <ul className="flex items-center justify-between gap-4 text-sm text-zinc-400">
                            <li className="flex flex-col items-center justify-center gap-2">
                                ...
                                {cabin.baths} {cabin.baths === 1 ? 'bathroom' : 'bathrooms'}
                            </li>
                            <li className="flex flex-col items-center justify-center gap-2">
                                <BedDoubleIcon className="text-primary size-4" />
                                {cabin.beds} {cabin.beds === 1 ? 'bed' : 'beds'}
                            </li>
                            <li className="flex flex-col items-center justify-center gap-2">
