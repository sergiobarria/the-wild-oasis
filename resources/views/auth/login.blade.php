    ...
    <section id="register"
        ...
        <form method="POST" action="{{ route('login.store') }}" class="space-y-4 w-full">
            ...
            <flux:field>
                ...
                <flux:input type="email" name="email" placeholder="iamawesome@email.com"/>
        ...
        @if($errors->has('email'))
            <div class="bg-red-500 w-full p-2 mt-2 rounded-lg text-sm text-white">
                {{ $errors->first('email') }}
            </div>
        @endif

        <p class="mt-4 text-sm text-zinc-400">Don't have an account yet?, <a href="{{ route('register') }}"
